...
In Druid 28.0.1, joins in native queries are implemented with a broadcast hash-join algorithm. This means
...
4. As of Druid 28.0.1, the join operator must evaluate the condition for each row. In the future, we expect
