...
You can run `apache-druid-33.0.0/bin/verify-java` to verify Java requirements for your environment.
...
Download the [33.0.0 release](https://druid.apache.org/downloads/) from Apache Druid. 
...
From the `apache-druid-33.0.0` package root, run the following command:
...
Druid stores all persistent state data, such as the cluster metadata store and data segments, in `apache-druid-33.0.0/var`.
Each service writes to a log file under `apache-druid-33.0.0/log`.
