...
* [**Coordinator**](./coordinator.md) service manages data availability on the cluster.
* [**Overlord**](./overlord.md) service controls the assignment of data ingestion workloads.
* [**Broker**](./broker.md) handles queries from external clients.
* [**Router**](./router.md) services are optional; they route requests to Brokers, Coordinators, and Overlords.
* [**Historical**](./historical.md) services store queryable data.
* [**MiddleManager**](./middlemanager.md) services ingest data.
...
For more details on process and server organization, please see [Druid Processes and Servers](./processes.md).
...
[_segments_](./segments.md). Each segment is a single file, typically comprising up to a few million rows of data. Since segments are
...
Queries first enter the [Broker](./broker.md), which identifies the segments with data that may pertain to that query.
The list of segments is always pruned by time, and may also be pruned by other attributes depending on how your
datasource is partitioned. The Broker will then identify which [Historicals](./historical.md) and
[MiddleManagers](./middlemanager.md) are serving those segments and distributes a rewritten subquery to each of those processes.
