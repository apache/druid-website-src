...
To use this Apache Druid extension, [include](./../extensions.md#loading-extensions) `druid-histogram` in the extensions load list.
...
> The Approximate Histogram aggregator is deprecated. Please use [DataSketches Quantiles](./datasketches-quantiles.md) instead which provides a superior distribution-independent algorithm with formal error guarantees.
...
For general histogram and quantile use cases, the [DataSketches Quantiles Sketch](./datasketches-quantiles.md) extension is recommended.
