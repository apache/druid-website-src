...
|spec|A Hadoop Index Spec. See [Ingestion](./index.md)|yes|
...
The spec file needs to contain a JSON object where the contents are the same as the "spec" field in the Hadoop index task. See [Hadoop Batch Ingestion](./hadoop.md) for details on the spec format.
