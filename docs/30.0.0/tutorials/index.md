...
You can run `apache-druid-30.0.0/bin/verify-java` to verify Java requirements for your environment.
...
Download the [30.0.0 release](https://www.apache.org/dyn/closer.cgi?path=/druid/30.0.0/apache-druid-30.0.0-bin.tar.gz) from Apache Druid. 
...
From the apache-druid-30.0.0 package root, run the following command:
...
Druid stores all persistent state data, such as the cluster metadata store and data segments, in `apache-druid-30.0.0/var`.
Each service writes to a log file under `apache-druid-30.0.0/log`.
