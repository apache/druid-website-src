...
In Druid 30.0.0, joins in native queries are implemented with a broadcast hash-join algorithm. This means
...
4. As of Druid 30.0.0, the join operator must evaluate the condition for each row. In the future, we expect
